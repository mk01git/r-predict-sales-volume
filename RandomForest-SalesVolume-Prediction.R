...
#check and make sure rendomForest package is installed.
if(!require("randomForest")){
  print("randomForest package is not installed")
  install.packages("randomForest")
}
#use randomForest library
...
# random forest - model 
model <- randomForest(formula = trainset$Volume ~ . , data = trainset, ntree = 5)
summary(model)
plot(model)
importance(model)
...
write.csv(newPrediction, "RandomForest-Result.csv")
